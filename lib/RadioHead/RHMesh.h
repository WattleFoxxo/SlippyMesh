...
public:
	...
	uint32_t             route[RH_MESH_MAX_MESSAGE_LEN - 2]; ///< List of node addresses visited so far. Length is implcit
    } MeshRouteDiscoveryMessage;
    ...
    bool recvfromAck(uint8_t* buf, uint8_t* len, uint32_t* source = NULL, uint32_t* dest = NULL, uint8_t* id = NULL, uint8_t* flags = NULL, uint8_t* hops = NULL);
    ...
    bool recvfromAckTimeout(uint8_t* buf, uint8_t* len,  uint16_t timeout, uint32_t* source = NULL, uint32_t* dest = NULL, uint8_t* id = NULL, uint8_t* flags = NULL, uint8_t* hops = NULL);
...
protected:
    ...
    virtual bool isPhysicalAddress(uint32_t* address, uint8_t addresslen);
