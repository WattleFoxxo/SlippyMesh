...
public:
    ...
    bool recvfromAck(uint8_t* buf, uint8_t* len, uint32_t* from = NULL, uint32_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
    ...
    bool recvfromAckTimeout(uint8_t* buf, uint8_t* len,  uint16_t timeout, uint32_t* from = NULL, uint32_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
...
private:
    ...
    uint8_t _seenIdsIndex;
    uint64_t _seenIds[32];

    void _addSeenID(uint32_t address, uint8_t id);
    uint8_t _getSeenID(uint32_t address);
};
