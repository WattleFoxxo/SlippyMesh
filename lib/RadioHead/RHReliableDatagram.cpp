        ...
        while ((timeLeft = timeout - (millis() - thisSendTime)) > 0)
	{
		...
		if (recvfrom(0, 0, &from, &to, &id, &flags)) // Discards the message
		{
			...
			return true;
				...
				&& (id == _getSeenID(from) /*_seenIds[from]*/))
		    {
...
////////////////////////////////////////////////////////////////////
bool RHReliableDatagram::recvfromAck(uint8_t* buf, uint8_t* len, uint32_t* from, uint32_t* to, uint8_t* id, uint8_t* flags)
{  
    uint32_t _from;
    uint32_t _to;
    uint8_t _id;
	...
	if (!(_flags & RH_FLAGS_ACK))
	{
	    ...
	    if ((RH_ENABLE_EXPLICIT_RETRY_DEDUP && !(_flags & RH_FLAGS_RETRY)) || _id != _getSeenID(_from) /*_seenIds[_from]*/)
	    {
		if (from)  *from =  _from;
		if (to)    *to =    _to;
		if (id)    *id =    _id;
		if (flags) *flags = _flags;
		_addSeenID(_from, _id);
		// _seenIds[_from] = _id;
		return true;
	    }
...
bool RHReliableDatagram::recvfromAckTimeout(uint8_t* buf, uint8_t* len, uint16_t timeout, uint32_t* from, uint32_t* to, uint8_t* id, uint8_t* flags)
{
...
void RHReliableDatagram::_addSeenID(uint32_t address, uint8_t id) {
	_seenIdsIndex += 1;
	...
	_seenIds[_seenIdsIndex] = ((uint64_t)address << 8) | id;
}
